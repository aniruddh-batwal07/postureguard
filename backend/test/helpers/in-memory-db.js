...
/**
 * Minimal in-memory stand-in for the MongoDB persistence surface used by the
 * backend stores (sessions, events, settings).
 *
 * Deterministic integration tests (e.g. the M4.2 hardware-loop test) need a
 * real ``createApp`` without depending on a running MongoDB. This object
 * exposes the same shape the stores rely on:
 *
 * - ``persistence.status()``        → ``{ connected: true }``
 * - ``persistence.getDb()``         → ``{ collection(name) }``
 * - collection: ``insertOne``, ``findOne``, ``findOneAndUpdate``,
 *   ``find()/sort()/limit()/toArray()`` (cursor), ``aggregate(...).toArray()``
 *
 * Operations mutate the in-memory arrays synchronously and resolve their
 * promises; documents are stored by reference so a test can seed collections
 * directly (``db.collection('sessions').insertOne({...})``).
 */
